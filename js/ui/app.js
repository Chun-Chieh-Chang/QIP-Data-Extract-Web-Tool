...
function bindEvents() {
  ...
  if (elements.closeConfigDialog)
    elements.closeConfigDialog.addEventListener("click", () =>
...
function renderPreviewTable(worksheet) {
  ...
  for (let r = 0; r < maxRows; r++) {
    ...
    for (let c = 0; c < maxCols; c++) {
      ...
      if (mergeMap[key]) {
        if (mergeMap[key].rowspan > 1)
          attrs += ` rowspan="${mergeMap[key].rowspan}"`;
        if (mergeMap[key].colspan > 1)
          attrs += ` colspan="${mergeMap[key].colspan}"`;
      }
...
function startRangeSelection(btn) {
  ...
  if (
    ...
    elements.previewTable
      .querySelectorAll(".selected, .selection-start")
      .forEach((el) => {
  ...
  elements.selectionModeText.textContent =
    selectionMode === "cavity"
      ? "選擇穴號範圍 (支援連帶選取數據區)"
      : "選擇數據範圍";
  elements.confirmSelection.disabled = true;
...
function confirmSelection() {
  ...
  // 檢查是否為合併操作：當在「穴號 ID 區域」選取多行時
  const isCavitySelection = selectionMode === "cavity";
  const isMultiRow = maxRow > minRow;

  if (isCavitySelection && isMultiRow) {
    // 合併模式：Pt 1 為 ID，其餘為數據
    const idRow = selectionStart.row;
    const idStartCell = XLSX.utils.encode_cell({ r: idRow, c: minCol });
    const idEndCell = XLSX.utils.encode_cell({ r: idRow, c: maxCol });
    const idRangeStr = `${idStartCell}:${idEndCell}`;

    // 決定數據區域 (排除 ID 行)
    let dataStartRow, dataEndRow;
    if (idRow === minRow) {
      dataStartRow = minRow + 1;
      dataEndRow = maxRow;
    } else if (idRow === maxRow) {
      dataStartRow = minRow;
      dataEndRow = maxRow - 1;
    } else {
      // 如果 Pt 1 在中間，則以 Pt 1 之後的區域為主（多數情況為標題在上方）
      dataStartRow = idRow + 1;
      dataEndRow = maxRow;
    }

    const dataStartCell = XLSX.utils.encode_cell({
      r: dataStartRow,
      c: minCol,
    });
    const dataEndCell = XLSX.utils.encode_cell({ r: dataEndRow, c: maxCol });
    const dataRangeStr = `${dataStartCell}:${dataEndCell}`;

    const groupId = parseInt(selectionTarget.split("-").pop());
    const idInput = document.getElementById(`cavity-id-${groupId}`);
    const dataInput = document.getElementById(`data-range-${groupId}`);

    if (idInput && dataInput) {
      idInput.value = idRangeStr;
      dataInput.value = dataRangeStr;
      idInput.classList.add("has-value");
      dataInput.classList.add("has-value");
      ...
      console.log(
        `[SmartMap] 穴組 ${groupId} 已自動拆分: ID=${idRangeStr}, Data=${dataRangeStr}`,
      );

      // 顯示狀態提示 (可選，這裡透過 console 確認)
      if (typeof showStatus === "function") {
        showStatus(
          `已自動識別 ID (${idRangeStr}) 與 數據 (${dataRangeStr})`,
          "success",
        );
      }
    }
...
function renderConfigList(searchTerm = "") {
  ...
  for (const c of filteredConfigs) {
    const originalIndex = configs.findIndex(
      (orig) => orig.name === c.name && orig.savedAt === c.savedAt,
    );
    const savedAt = new Date(c.savedAt).toLocaleString("zh-TW", {
      hour12: false,
      month: "2-digit",
      day: "2-digit",
      hour: "2-digit",
  ...
  if (configs.length > 5) {
    ...
    elements.configList.innerHTML =
      infoHtml + `<ul class="space-y-2">${html}</ul>`;
  } else {
...
function importConfigurations(e) {
  ...
  reader.onload = function (event) {
    try {
      ...
      if (
        confirm(
          `確定要導入 ${importedConfigs.length} 筆配置嗎？這將會與現有配置合併。`,
        )
      ) {
        const currentConfigs = JSON.parse(
          localStorage.getItem("qip_configs") || "[]",
        );
        ...
        for (const imp of importedConfigs) {
          const exists = merged.find(
            (c) => c.name === imp.name && c.savedAt === imp.savedAt,
          );
          if (!exists) {
            merged.push(imp);
            addedCount++;
        ...
        alert(
          `導入成功！新增了 ${addedCount} 筆配置，總計 ${merged.length} 筆。`,
        );
...
function clearAllConfigurations() {
  ...
  if (
    confirm(
      `危險：確定要刪除所有 ${count} 筆已保存的配置嗎？此操作無法恢復！\n\n建議先執行「導出」備份。`,
    )
  ) {
...
function updateStartButton() {
  ...
  const hasCavityId =
    document.getElementById("cavity-id-1")?.value?.trim() !== "";
  const hasDataRange =
    document.getElementById("data-range-1")?.value?.trim() !== "";
...
async function startProcessing() {
  ...
  try {
    ...
    processingResults = await processor.processMultipleWorkbooks(
      workbooks,
      (progress) => {
...
function showResults(results) {
  ...
  if (results.hasErrors) {
    ...
    elements.errorList.innerHTML = results.errors
      .map(
        (e) =>
