...
class DataExtractor {
  ...
  static extractInspectionItemsFromGroup(worksheet, groupConfig) {
    ...
    try {
      const idRangeParsed = DataValidator.parseRangeString(
        groupConfig.cavityIdRange,
      );
      const dataRangeParsed = DataValidator.parseRangeString(
        groupConfig.dataRange,
      );
      ...
      for (
        let rowOffset = 0;
        rowOffset <= dataRangeParsed.endRow - dataRangeParsed.startRow;
        rowOffset++
      ) {
        ...
        for (
          let colOffset = 0;
          colOffset < idRangeParsed.endCol - idRangeParsed.startCol + 1;
          colOffset++
        ) {
          ...
          // 初始化穴號數據，確保即使沒有數值也會在 allCavities 中記錄此穴號
          if (!data.hasOwnProperty(cavityId)) {
            data[cavityId] = null;
          }
      ...
      console.error(
        "DataExtractor.extractInspectionItemsFromGroup error:",
        error,
      );
    }
          ...
          for (let r = 1; r <= 2; r++) {
            for (let c = 15; c <= 21; c++) {
              ...
              if (
                cell &&
                cell.v &&
                String(cell.v).trim() !== "0" &&
                String(cell.v).trim() !== "False"
              ) {
